...
- **输入门禁**：读当前 task 的 `prd.md`，**按顺序判定，命中即停**：
...
```
Phase 1 门禁 + 读本片需求 → Phase 2 出 2–3 个方案 → Phase 3 走查 + 选案(≤2 轮) → Phase 4 回写 prd.md
```
...
| 问题记录 | 4 字段（现象/判定/动作/落点），不套 `design-review` 的 9 字段台账 |

...
> **本节是触发条件的权威源。** 手册和检查点指过来的就是这里——别在别处另列一份短的，那样迟早会漏掉某一种情况。

四种，满足任一条就值得喊一次：

| # | 什么时候 | 判据 |
|---|---|---|
| 1 | **承重决策刚定稿，准备建第一批 task 之前** | 数据所有权、权限与信任边界、模块切分、长期契约——这类决定改起来贵，值得在开工前单独审一轮 |
| 2 | **已经陷在"审不完"里出不来** | 换了几个 session / 几个 agent 反复审同一份东西，问题越修越多，每轮都冒新的，说不出什么时候算审完 |
| 3 | **项目级检查点发现承重决策中途失效** | 判据是「修它要不要动**已经做完的好几片**」。典型信号是连着两片都在绕同一个东西——那通常是承重层的问题，不是那两片各自没做好。这不是记一条 finding 能解决的 |
| 4 | **高风险范围变更后** | 鉴权、数据迁移、不可逆操作、涉钱 |

第 2 条是它真正的用途：**它治的不是"审得不够细"，是"停不下来"**。

**怎么触发**：用户主动喊——「审一下设计」「这个设计能开工了吗」「设计审不完了」「反复评审停不下来」。逐片流程里没有任何一步会自动调它，这是设计如此。
...
逐片推进时本 skill 的触发面很窄：大部分切片不需要独立的设计准入，Trellis 的 planning summary 门禁就够了。上面四条里，**1 和 3 是承重决策的两个时刻**（刚定稿 / 中途失效），**2 是它真正的用途**，4 是范围变更后的补审。都不占就别开评审。
...
**未冻结输入或未读/建台账时不得进入评审**——这是硬检查点，不是建议。（本框架「门禁」一词专留给 Trellis 的 planning summary 批准，那是唯一能拦住 `task.py start` 的东西；本 skill 拦的是评审本身的入口。）

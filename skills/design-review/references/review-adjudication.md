...
8. **Stop on the rule, not on exhaustion.** Two rounds of pure design review, maximum. Build when Accepted P0 is empty, every Accepted P1 has a disposition, at least one vertical slice is implementable, and **one full round has produced no new Accepted P0/P1**. Past that budget, route feedback into slice implementation and tests — cheaper feedback, and truer.

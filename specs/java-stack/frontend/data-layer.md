...
> The SPA's data layer: the provider as the only path to the backend, generated types, the cache across tabs, reuse order, hooks, theme tokens and deep links.
>
...
- *Why:* the type layer cannot carry this constraint — `schema.d.ts` widens these parameters back to `string` and `number`. The backend half is in [`../backend/write-path.md`](../backend/write-path.md) §10.
...
- **The full precedence between the approved hi-fi, `MASTER.md`, shadcn MCP and the UI/UX rules is in [`index.md`](index.md), under "What decides what".** Where the approved hi-fi depicts something it governs; where it is silent, the rules fill in.

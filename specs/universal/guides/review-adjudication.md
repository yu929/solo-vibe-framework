...
>
> **正式设计准入评审**（十条纪律、P0–P3、九字段证据格式、停止规则）不在这里——它在 `design-review` skill 的 `references/review-adjudication.md`，只在触发评审时加载。两边管的事不同，不是同一份正文的两个副本。
...
**为什么只 4 个字段**：这类发现的形态是「跑的时候发现 X」，不是「设计文档 §3.3 规定了 Y 会导致 Z」。套设计准入那套九字段，等于要求为每条实战观察写举证材料——那会让人干脆不记，或者让 AI 编出证据来凑格式。

**判据（要不要记）**：下次别的 session 遇到同类情况，会不会因为 spec 没写而重犯？会就记。**大部分 bug 不需要记**——一个 bug 说明代码没做到已有约定，约定本身没变。

**落点**：项目自己的 `findings/` 目录或已知问题文件，按目标仓库约定。升级进 spec 时，把状态改成「已升级」并留下指向那条 spec 的链接。

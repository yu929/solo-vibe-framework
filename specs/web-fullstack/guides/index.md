...
**它不含完整评审协议**（十条纪律、P0–P3、九字段证据格式、停止规则）——那些在 `design-review` skill 里，只在触发评审时加载。这里只有每天都会用到的两条。

## 完工质量门

这三份清单本身不构成质量门——具体的 typecheck / lint / test / build 命令在各仓库自己的约定里。
...
- **改这三份文件要去改框架仓的 `specs/universal/guides/`（权威源）**，各轨模板里的是它的生成副本，直接改会在下次同步时被覆盖。

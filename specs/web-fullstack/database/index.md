...
**唯一受控例外**是异构子服务的 worker，范围与不变量定义在 [`../backend/index.md`](../backend/index.md) §6.1，**只在那里定义一次**。要点：worker 的入参只有 job id，job 的归属在创建时由用户作用域路径 + RLS 钉死；worker 读用户表一律经显式校验归属的数据库函数，不按外部传入的主键取数。别处想用 service-role 时不要照抄「子服务可以」这个结论。
